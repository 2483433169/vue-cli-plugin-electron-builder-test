...
export default {
  ...
  methods: {
    ...
    /**
     * info
     * info = [{type:"",regionColor:"red",region:[{x:0,y:0},{x:0.5,y:0.5}],areaName:"测试"}]
     *
     */
    drawWithInfo () {
    ...
    canvasMove (e) {
      if (this.canvasMoveUse && this.canDraw) {
        ...
        if (this.lineType === 'rec') {
          ...
          this.context.beginPath()
          const x = this.beginRec.x
          const y = this.beginRec.y
          const x1 = canvasX - this.beginRec.x
          const y1 = canvasY - this.beginRec.y
          const position = [x, y, x1, y1]
          //   console.log(position, "position");

          this.context.rect(...position)
